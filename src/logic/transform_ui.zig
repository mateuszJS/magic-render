...
fn get_points_of_line(texture: Texture, transform_line: TransformLine, render_scale: f32) struct { Point, Point } {
...
pub fn get_transform_ui(
    ...
    for (resize_lines) |transform_line| {
        ...
        const p1, const p2 = get_points_of_line(texture, transform_line, render_scale);
...
pub fn get_transform_ui_pick(buffer: *[PICK_BORDER_BUFFER_SIZE]f32, texture: Texture, render_scale: f32) void {
    var i: usize = 0;
    for (resize_lines) |transform_line| {
        const p1, const p2 = get_points_of_line(texture, transform_line, render_scale);
