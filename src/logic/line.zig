...
pub const DRAW_NUM_VERTICIES: usize = 2 * Triangle.DRAW_NUM_VERTICIES;
pub const PICK_NUM_VERTICIES: usize = 6 * 5;

pub fn get_vertex_data(buffer: *[DRAW_NUM_VERTICIES]f32, start: anytype, end: anytype, width: f32, color: [4]f32) void {
...
pub fn get_vertex_data_pick(buffer: *[PICK_NUM_VERTICIES]f32, start: anytype, end: anytype, width: f32, id: f32) void {
    const half_width = width / 2.0;
    const parallel_angle = std.math.atan2(end.y - start.y, end.x - start.x);
    const angle = parallel_angle + std.math.pi / 2.0; // perpendicular angle
