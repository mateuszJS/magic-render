...
fn pseudoDistance(pixelPos: vec2f, curveIndex: u32) -> f32 {
  ...
  for (var i: u32 = 0; i <= samples; i++) {
    ...
    minDistance = min(minDistance, distance(pointOnCurve, pixelPos));
  }
...
@fragment fn fs(in: VertexOutput) -> @location(0) vec4f {
  ...
  // 1. Calculate global winding number
  ...
  // 2. Find the closest unsigned distance for each channel
  var minRedDist = 1e38;
  ...
  // 3. Apply the sign to each channel's distance
  let signedRed = sign * minRedDist;
  let signedGreen = sign * minGreenDist;
  let signedBlue = sign * minBlueDist;

  // 4. The MSDF is the median of the three signed channel distances.
  let finalSDF = median(signedRed, signedGreen, signedBlue);

  // Normalize the distance to a 0-1 range for visualization.
  // A value of 0.5 is the boundary.
