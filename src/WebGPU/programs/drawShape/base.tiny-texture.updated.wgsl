...
fn getSample(pos: vec2f) -> Sample {
  ...
  let angle_threshold = PI * 0.7 * 100;
  ...
  // let w00 = select(0.0, (max_d - d00) / max_d, _diff00 < BILINEAR_T_THRESHOLD && abs(ndiff00) < angle_threshold);
  // let w10 = select(0.0, (max_d - d10) / max_d, _diff10 < BILINEAR_T_THRESHOLD && abs(ndiff10) < angle_threshold);
  // let w01 = select(0.0, (max_d - d01) / max_d, _diff01 < BILINEAR_T_THRESHOLD && abs(ndiff01) < angle_threshold);
  // let w11 = select(0.0, (max_d - d11) / max_d, _diff11 < BILINEAR_T_THRESHOLD && abs(ndiff11) < angle_threshold);
