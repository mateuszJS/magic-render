...
fn getSample(pos: vec2f) -> Sample {
  ...
  let naerest_tan = g_to_bezier_tangent(nearest_g);
  ...
  let angle_threshold = PI * 0.4;

  let w00 = select(0.0, (1.0 - fract_pos.x) * (1.0 - fract_pos.y), abs(ndiff00) < angle_threshold);
  let w10 = select(0.0, fract_pos.x         * (1.0 - fract_pos.y), abs(ndiff10) < angle_threshold);
  let w01 = select(0.0, (1.0 - fract_pos.x) * fract_pos.y,         abs(ndiff01) < angle_threshold);
  let w11 = select(0.0, fract_pos.x         * fract_pos.y,         abs(ndiff11) < angle_threshold);
